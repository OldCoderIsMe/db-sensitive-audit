...
- ✅ **完整测试覆盖** - 16个单元测试，55%测试覆盖率
- 🚀 **即插即用** - 提供 `run_audit.sh` 一键运行脚本  
- 📦 **零配置启动** - 支持交互式配置，无需预先准备配置文件
- 🔧 **生产级别** - 支持批量数据库审计，已验证可处理128+数据表
- 📊 **专业报告** - 生成企业级Excel审计报告，支持风险等级分类
...
```bash
# 克隆项目
git clone https://github.com/OldCoderIsMe/db-sensitive-audit.git
...
# 2. 一键运行（使用便捷脚本）
./run_audit.sh audit -c "demo_db,your_host,3306,your_user,your_password"
...
```bash
# 使用配置文件
./run_audit.sh audit -f config/database_config.txt
...
- 🐍 **确认Python版本** 兼容性（支持Python 3.8+，推荐3.9+）
